        ...
        <div class="card">
          ...
          <el-form v-else :model="formData" label-width="100px">
            <el-form-item label="主标题">
              <el-input v-model="formData.heroTitle" placeholder="请输入主标题，如：新闻动态" />
            </el-form-item>
          ...
          <div class="preview-wrapper">
            ...
            >
              ...
              <div class="hero-content">
                <h1>{{ formData.heroTitle || '新闻动态' }}</h1>
                <p>{{ formData.heroSubtitle || '了解公司最新资讯与行业动态' }}</p>
              </div>
            </div>
          </div>
        </div>
...
const load = async () => {
  try {
    const res = await siteApi.getNewsHero()
    const d = res.data.data || {}
    formData.value = {
      heroTitle: d.heroTitle || '新闻动态',
      heroSubtitle: d.heroSubtitle || '了解公司最新资讯与行业动态',
...
const saveFormData = async () => {
  saving.value = true
  try {
    await siteApi.saveNewsHero(formData.value)
