        ...
        <div class="card">
          ...
          <div v-if="!isEditing" class="text-display">
            ...
            <div class="text-item">
              ...
              <div class="text-value image-preview">
                <img v-if="formData.backgroundImage" :src="formData.backgroundImage" alt="背景图片" class="preview-img" />
          ...
          <el-form v-else :model="formData" label-width="100px">
            <el-form-item label="主标题">
              <el-input v-model="formData.companyName" placeholder="请输入主标题，如：关于我们" />
            </el-form-item>
            <el-form-item label="副标题">
              <el-input v-model="formData.companyDesc" type="textarea" :rows="3" placeholder="请输入副标题描述文字" />
            </el-form-item>
            <el-form-item label="背景图片">
              <div class="upload-wrapper">
                <el-upload
                  ...
                  accept="image/*"
                >
                  <img v-if="formData.backgroundImage" :src="formData.backgroundImage" class="uploaded-img" />
                ...
                <div class="upload-actions">
                  <el-button v-if="formData.backgroundImage" type="danger" size="small" @click="removeImage">移除图片</el-button>
                </div>
              </div>
...
const formData = ref({ companyName: '', companyDesc: '', backgroundImage: '', bgPosition: 'center' })
...
const previewStyle = computed(() => {
  ...
  if (formData.value.backgroundImage) {
    style.backgroundImage = `url(${formData.value.backgroundImage})`
  }
...
const handleUpload = (options) => {
  ...
  api.post('/api/upload', formDataUpload).then(res => {
    if (res.data.code === 200) {
      formData.value.backgroundImage = res.data.data.url
...
const removeImage = async () => {
  if (!formData.value.backgroundImage) return
  try {
    await api.delete('/api/upload/delete', { params: { fileUrl: formData.value.backgroundImage } })
  } catch (e) {
  }
...
const load = async () => {
  try {
    const res = await api.get('/api/site/about-hero')
    const d = res.data.data || {}
    formData.value = {
      companyName: d.companyName || '四川国驰恒创科技有限公司',
      companyDesc: d.companyDesc || '核心技术源自美国辛辛那提大学博士团队，专注于提供新能源电池快充技术解决方案。',
...
const saveFormData = async () => {
  saving.value = true
  try {
    await api.put('/api/site/about-hero', formData.value)
