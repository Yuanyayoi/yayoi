        ...
        <div class="card">
          ...
          <el-form v-else :model="formData" label-width="100px">
            <el-form-item label="主标题">
              <el-input v-model="formData.heroTitle" placeholder="请输入主标题，如：联系我们" />
            </el-form-item>
          ...
          <div class="preview-wrapper">
            ...
            >
              ...
              <div class="hero-content">
                <h1>{{ formData.heroTitle || '联系我们' }}</h1>
                <p>{{ formData.heroSubtitle || '有任何问题，欢迎随时与我们联系。' }}</p>
              </div>
            </div>
          </div>
        </div>
...
const load = async () => {
  try {
    const res = await siteApi.getContactHero()
    const d = res.data.data || {}
    formData.value = {
      heroTitle: d.heroTitle || '联系我们',
      heroSubtitle: d.heroSubtitle || '有任何问题，欢迎随时与我们联系。',
...
const saveFormData = async () => {
  saving.value = true
  try {
    await siteApi.saveContactHero(formData.value)
